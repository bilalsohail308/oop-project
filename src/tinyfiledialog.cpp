...
std::string openFileDialoga() {
    ...
    if (GetOpenFileNameW(&ofn)) { // Use wide version of GetOpenFileName
        return wideToNarrowa(std::wstring(fileName)); // Convert wide string to narrow and return it
    }
