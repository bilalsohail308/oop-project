...
using namespace std;
    ...
    Comment::Comment(const std::string& text, float yPosition) {
        font.loadFromFile("F:\\sem3\\OOP\\oop-project\\src\\Gossip\\arial.ttf");
...
Post::Post(const std::string& content, const sf::Texture& texture, double yPosition,const std::string& a)
    ...
    fonta.loadFromFile("F:\\sem3\\OOP\\oop-project\\src\\Gossip\\arial.ttf");
    text.setFont(fonta);
...
PostFeed::PostFeed(sf::RenderWindow* win)
    ...
    font.loadFromFile("F:\\sem3\\OOP\\oop-project\\src\\Marketplace\\Arial.ttf");
    filePath="F:\\sem3\\OOP\\oop-project\\src\\Marketplace\\posts.txt";
    initializeUI();
    loadItemsFromFile();
}
